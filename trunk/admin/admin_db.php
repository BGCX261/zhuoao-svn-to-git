...
elseif($action=='save_back'){
	...
	if($sql){
		...
		show_htm('文件'.$fl_name.'备份完成','?action=save_back&page='.($page+1).'&r_start='.$r_start.'&table_id='.$i.'&sql_size='.$sql_size.'&rand='.$rand.'&dir='.$dir);
	}else{
		msg("数据备份完成",'?');
	}
}
