...
?>
$get='#<?php echo $get;?>';
$(self.parent.document).find($get).val($file_val).focus();
<?php }?>
});
...
?>
 ...
 <table cellpadding="0" cellspacing="0" width="100%" class="table_info1">
 	<thead>
		<tr><th style="5%">选择</th><th style="width:5%">ID</th><th style="width:25%">文件说明</th><th style="width:30%">文件地址</th><th style="width:15%">格式</th><th style="15%">上传时间</th><th style="width:5%">下载数</th></tr>
	</thead>
	<tbody id="show_list">
...
foreach($rel as $k=>$v){
?>
	<tr><td style="width:5%; text-align:center"><input name="sl_rd" id="file_rd" type="radio" <?php if(!$k){echo 'checked="checked"';}?> value="<?php echo CMS_SELF.$v['file_path'];?>"/></td><td style="width:5%;text-align:center" align="center"><?php echo $v['id'];?></td><td style="width:25%;text-align:center" align="center"><?php echo $v['file_info'];?></td><td scope="width:25%;text-align:center" align="center"><?php echo $v['file_path'];?></td><td style="width:15%; text-align:center"><?php echo $v['file_ext'];?></td><td style="width:15%; text-align:center"><?php echo date('Y-m-d H:m:s',$v['file_time']);?></td><td style="width:5%; text-align:center"><?php echo $v['hits'];?></td></tr>
<?php
}
?>
