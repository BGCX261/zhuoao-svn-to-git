...
elseif($action=='save_field'){
	...
	if(strlen($field_value)>1000){msg('<span style="color:red">字段类型太长,请缩短</span>');}
...
elseif($action == 'save_import'){
	...
	msg('字段导出完成！请到data/backup目录下下载导出文件！','?action=form');
}
...
elseif($action=='del_list'){
	...
	msg('表单信息成功删除','?action=form_list&id='.$form_id);
}
...
elseif($action=='del_all'){
	...
	msg('表单成功删除！','?action=form_list&id='.$form_id);
}
?>
