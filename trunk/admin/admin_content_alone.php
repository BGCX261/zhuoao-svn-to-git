...
elseif($action=='save_content'){
	...
	*/
	...
	msg('内容添加成功','?action=content_list&lang='.$lang);
}
...
elseif($action=='save_edit_content'){
   ...
   $key_words=empty($key_words)?'':cn_substr($key_words,150);
	...
	msg('内容修改成功,<a href="admin_content_alone.php?action=content_list">返回内容列表</a>',"admin_content_alone.php?action=content_list&lang=".$lang);
...
elseif($action == 'del')
{
	...
	if(empty($id))
	{
		msg('参数发生错误，请重新操作','admin_content_alone.php?action=content_list&lang='.$lang);
	}
	$table = 'alone';
	$GLOBALS['mysql']->query('delete from '.DB_PRE."maintb where id={$id}");
	$GLOBALS['mysql']->query('delete from '.DB_PRE."{$table} where id={$id}");
	msg('内容删除完成','admin_content_alone.php?action=content_list&lang='.$lang);
}
