...
elseif($action=='save_flash_ad'){
	...
	msg('主广告添加完成','?$cate_id='.$cate_id.'&action=flash_ad&lang='.$lang);
}
//修改主广告
elseif($action=='edit'){
	$id=intval($_GET['id']);
...
elseif($action=='save_edit'){
	...
	msg('主广告修改成功','?action=flash_ad&lang='.$lang.'&cate_id='.$cate_id);
}
//删除主广告
elseif($action=='del'){
	$id=intval($_GET['id']);
