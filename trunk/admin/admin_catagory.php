...
elseif($action=='add'){
	...
	if(!check_str($cate_fold_name,'/^\w+$/')){
		msg('<span style="color:red">请使用字母数字或_组合作为栏目目录名</span>');
	}
	if(!empty($custom_url)){
	if(empty($custom_url)&&!check_str($custom_url,'/^[a-z0-9A-Z_-]*[a-z0-9A-Z_-]$/')){
	...
	if(strlen($cate_name)>60){msg('<span style="color:red">【栏目名称】太长,请缩短</span>');}
	if(strlen($cate_fold_name)>60){msg('<span style="color:red">【栏目目录名】太长,请缩短</span>');}
	if(strlen($cate_order)>60){msg('<span style="color:red">【排列顺序】太长,请缩短</span>');}
	if(strlen($cate_tpl_list)>60){msg('<span style="color:red">【列表页模板】太长,请缩短</span>');}
	if(strlen($cate_tpl_content)>60){msg('<span style="color:red">【内容页模板】太长,请缩短</span>');}
	...
	$cate_fold_name=$cate_fold_name.'-'.$lang;
	$sql="insert into ".DB_PRE."category (cate_name,cate_mb_is,cate_hide,cate_channel,cate_fold_name,cate_order,cate_tpl,cate_tpl_list,cate_tpl_content,cate_title_seo,cate_key_seo,cate_info_seo,lang,cate_parent,cate_html,cate_nav,cate_url,cate_is_open,form_id,custom_url,cate_pic1,cate_pic2,cate_pic3,cate_content,temp_id,list_num) values ('".$cate_name."',0,".$cate_hide.",".$cate_channel.",'".$cate_fold_name."',".$cate_order.",".$cate_tpl.",'".$cate_tpl_list."','".$cate_tpl_content."','".$cate_title_seo."','".$cate_key_seo."','".$cate_info_seo."','".$lang."',".$parent.",".$cate_html.",'".$cate_nav."','".$cate_url."',".$cate_is_open.",".$form_id.",'".$custom_url."','".$cate_pic1."','".$cate_pic2."','".$cate_pic3."','".$cate_content."',".$temp_id.",".$list_num.")";
	...
	//create_nav_xml($lang);
	$GLOBALS['cache']->cache_category_all();
	msg('栏目添加成功','?action=category_add&lang='.$lang);
}
...
elseif($action=='save_xg'){
	...
	$sql="update ".DB_PRE."category set cate_name='".$cate_name."',cate_mb_is=0,cate_hide=".$cate_hide.",cate_channel=".$cate_channel.",cate_order=".$cate_order.",cate_tpl=".$cate_tpl.",cate_tpl_index='',cate_tpl_list='".$cate_tpl_list."',cate_tpl_content='".$cate_tpl_content."',cate_title_seo='".$cate_title_seo."',cate_key_seo='".$cate_key_seo."',cate_info_seo='".$cate_info_seo."',cate_nav='".$cate_nav."',cate_html=".$cate_html.",cate_url='".$cate_url."',cate_is_open=".$cate_is_open.",form_id=".$form_id.",cate_pic1='".$cate_pic1."',cate_pic2='".$cate_pic2."',cate_pic3='".$cate_pic3."',cate_content='".$cate_content."',temp_id=".$temp_id.",list_num=".$list_num." where id=".$id." and lang='".$lang."'";
	$GLOBALS['mysql']->query($sql);
	$GLOBALS['cache']->cache_category($parent,$lang);
	//更新栏目列表
	$sql="select c.id,c.list_num,c.cate_pic1,c.cate_pic2,c.cate_pic3,c.cate_content,c.temp_id,c.custom_url,c.cate_channel,c.is_content,c.cate_url,c.cate_is_open,c.cate_html,c.cate_nav,c.cate_fold_name,c.cate_order,c.cate_hide,c.cate_tpl,c.cate_name,c.lang,c.cate_parent,COUNT(s.id) as haschild from ".DB_PRE."category as c left join ".DB_PRE."category as s on c.id=s.cate_parent where c.lang='".$lang."' group by c.id order by c.cate_order,c.id desc";
