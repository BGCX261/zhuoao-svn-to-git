...
function form_fields($id,$path='',$edit='false'){
	...
	$opt='';
		foreach($field as $key=>$value){
			...
			if($value['form_id']==$id){
				...
				if(!in_array($value['field_type'],array('checkbox','radio'))){
				$help=empty($value['info'])?"":"<span title=\"{$value['field_info']}\" class=\"help\"></span>";
				$opt.="<tr><td class=\"w1\" style=\"width:30%\">{$help}{$value['use_name']}:</td><td style=\"width:70%\">".$value['field_type']($value['field_name'],$v,1)."{$note}</td></tr>";
				}else{
				if($edit=='false'){$v='';}
				$opt.="<tr><td class=\"w1\" style=\"width:30%\"><span title=\"{$value['field_info']}\" class=\"help\"></span>{$value['use_name']}:</td><td style=\"width:70%\">".$value['field_type']($value['field_name'],$value['field_value'],$v)."{$note}</td></tr>";
				}
			}
		}
	...
	$str.=$js;
	$str.="}</script>";
	$str.="<form action=\"{$path}order/order_save.php\" class=\"order_form\" name=\"form_form\" method=\"post\" onsubmit=\"return check_form();\">";
	...
	if($arc_title){	
		$str.="<tr><td style=\"width:30%\" class=\"w1\">产品（职位）：</td><td style=\"width:70%\">{$arc_title}</td></tr>";
	}
	$str.=$opt;	
	$str.="<tr><td style=\"width:30%\"></td><td style=\"width:70%\"><input type=\"hidden\" value=\"{$f_id}\" name=\"f_id\"/><input type=\"submit\" name=\"submit\" class=\"pt_go\" value=\"提交\"/><input type=\"reset\" name=\"reset\" class=\"pt_reset\" value=\"重置\"/></td></tr>";
	$str.="</table></form>";
	}
	return $str;
}
...
function upload_pic($f_name,$f_value){
		...
		$str='<input name="fields['.$f_name.']" value="'.$f_value.'" style="width:30%; display:block; float:left; margin-top:55px;" id="'.$f_name.'" />
		  <p style="margin-top:55px;" class="admin_up_pic"><a href="admin_pic_upload.php?type=radio&get='.$f_name.'" id="pic_'.$f_name.'">上传图片</a></p><span id="show_'.$f_name.'" class="admin_show_pic"><img src="'.$pic_str.'"  height="120" width="120"/></span><script type="text/javascript">$(\'#pic_'.$f_name.'\').wBox({title:\'附件\',requestType: "iframe",target:$(\'#pic_'.$f_name.'\').attr(\'href\'),iframeWH:{width:800,height:400}});</script>';
	return $str;
}
//多图上传
...
function upload_file($f_name,$f_value){
	$str="<input name=\"fields[{$f_name}]\" value=\"{$f_value}\" style=\"width:30%;margin-right:8px;\" id=\"{$f_name}\"/>";
...
function get_channel_list($id=''){
	...
	$ck=($id=='-9')?"selected=\"selected\"":'';
	$str.="<option title=\"order\" value=\"-9\" {$ck}>表单模块</option>";
	$str.="</select>";
	return $str;
}
//生成栏目列表
...
*return $arr-原始图和缩略图
*/
function up_img($file,$size,$type,$thumb=0,$thumb_width='',$thumb_height='',$logo=1){
...
function up_file($file,$size,$type,$path='',$name=''){
		...
		$path=empty($path)?CMS_PATH."upload/file/".date('Ymd').'/':CMS_PATH.$path.'/';
		if(!file_exists($path)){
			@mkdir($path);
		}
		$name=empty($name)?date('YmdHis'):$name;
		$file_name=$path.$name.'.'.$pic_name['extension'];
		unset($pic_name);
		if(!move_uploaded_file($file['tmp_name'],$file_name)){
			msg('文件上传失败');
		}
		$return_name=str_replace(CMS_PATH,"",$file_name);
...
function page($url='',$page,$query='',$totalnum,$totalpages,$cate_id='',$ishtml=0){
	...
	if($_confing['web_html'][0]&&!$ishtml){
		...
		$str.=($page>1)?'<a href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/index.html">'.$language['pagehome'].'</a>':'<span class="off">'.$language['pagehome'].'</span>';
		if($page>1){
			
			if(($page-1)==1){
				$str.='<a class="p" href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/index.html">'.$language['pagapre'].'</a>';
			}else{
				$str.='<a class="p" href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/list_'.$pre_page.'.html">'.$language['pagapre'].'</a>';
			}
		}else{
		...
		for($i=$page_star;$i<=$page_end;$i++){
			...
			if($i==1){
				$str.='<a class="'.$class.'" href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/index.html">1</a>';
			}else{
				$str.='<a class="'.$class.'" href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/list_'.$i.'.html">'.$i.'</a>';
			}
			if($i==$page_end&&$i!=$totalpages){$str.='...';}
		}
		
		for($i=1;$i<=$totalpages;$i++){
			$op.=($i==1)?"<option value=\"index.html\" {$sel}>{$i}</option>":"<option value=\"list_{$i}.html\" {$sel}>{$i}</option>";
		}
		
		if($page<$totalpages){
			$str.='<a class="p" href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/list_'.$next_page.'.html">'.$language['pagenext'].'</a>';
		}else{
			$str.='<span class="off">'.$language['pagenext'].'</span>';
		}
		$str.=($page<$totalpages)?'<a href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/list_'.$totalpages.'.html">'.$language['pageend'].'</a>':'<span class="off">'.$language['pageend'].'</span>';
...
function get_position($parent,$cate,$path,$list_php=''){
	...
	foreach($cate as $k=>$v){
		if($v['id']==$parent){
			$cate_path=($v['cate_html']&&$_confing['web_html'][0])?$path.'htm/'.$v['cate_fold_name']:$path.$list_php.'?id='.$v['id'];
...
//获取表单列表
function get_form_list($id=''){
	if(!empty($id)){$rel=$GLOBALS['mysql']->fetch_asc("select form_id from ".DB_PRE."category where id=".intval($id));}
	if(file_exists(DATA_PATH.'cache_form/form.php')){include(DATA_PATH.'cache_form/form.php');}
	if(empty($form)){return "没有表单，请先添加表单";}
	$form_option = '<select name="order">';
	foreach($form as $k=>$v){
		if(!$v['is_disable']){
			$sl=($v['id']==$rel[0]['form_id'])?'selected="selected"':"";
